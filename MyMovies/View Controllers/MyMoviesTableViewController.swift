...
class MyMoviesTableViewController: UITableViewController, NSFetchedResultsControllerDelegate {
    ...
    lazy var fetchedResultsController: NSFetchedResultsController<Movie> = {
        ...
        let context = CoreDataTask.shared.context
        
        let fetchResultsController = NSFetchedResultsController(fetchRequest: fetchRequest, managedObjectContext: context, sectionNameKeyPath: "hasBeenWatched", cacheName: nil)
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            let movie = fetchedResultsController.object(at: indexPath)
            self.movieController.delete(movie)
        }
    }
