...
class MyMoviesTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            guard let movies = fetchedResultsController.fetchedObjects else { return }
            // delete from server
            movieController.deleteMovieFromServer(movie: movies[indexPath.row])
            // delete from coreData + save
            CoreDataManager.shared.mainContext.delete(movies[indexPath.row])
            do {
                try CoreDataManager.shared.mainContext.save()
            } catch {
                NSLog("Error saving delete of managed object context: \(error)")
                return
            }
        }
    }
