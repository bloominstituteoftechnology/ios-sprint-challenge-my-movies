...
class MovieController
{
    ...
    private let baseMovieDatabaseURL = URL(string: "https://api.themoviedb.org/3/search/movie")!
	private let baseFirebaseURL = URL(string: "https://pushier-and-postier.firebaseio.com/")!

    var searchedMovies: [MovieStub] = []
	...
	func addOrUpdateFromStub(_ stub:MovieStub, moc:NSManagedObjectContext) throws
	{
		...
		moc.perform {
			...
			if let movie = movie {
				movie.applyStub(stub)
			} else {
				_ = Movie(stub.title, stub.hasWatched ?? false, stub.identifier, moc:moc)
			}
		}
	...
	func fetchRemote(_ completion:@escaping CompletionHandler = EmptyHandler)
	{
		...
		URLSession.shared.dataTask(with: req) { data, _, error in
			...
			do {
				let stubs = try JSONDecoder().decode([String:MovieStub].self, from:data)
				for (_, stub) in stubs {
	...
	{
        var components = URLComponents(url: baseMovieDatabaseURL, resolvingAgainstBaseURL: true)
